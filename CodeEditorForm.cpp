...
void CodeEditorForm::OnPaint() {
	...
	if (this->regionController->GetIsBeing() == TRUE){
		...
		rgnText = CString(this->regionController->contents.c_str());
		
		dc.FillRgn(&this->regionController->selectedRgn, &brush);
		dc.SetTextColor(RGB(255, 255, 255));
		dc.SetBkColor(RGB(37, 111, 253));
		dc.TextOut(this->regionController->firstX, this->regionController->firstY, rgnText, rgnText.GetLength());
		dc.PaintRgn(&this->regionController->selectedRgn);
